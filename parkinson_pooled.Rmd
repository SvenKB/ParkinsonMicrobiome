...
saveRDS(dat,"C:\\Users\\kleinebardenhorst\\Documents\\Projects\\MicrobiomeBenchmark\\example_data.RDS")
...
N <- length(y)
x <- full.dat %>% dplyr::select(status) %>% mutate(status = case_when(status=="HC"~0,
                                                                      status=="PD"~1)) %>% pull
...
m <- sampling(sm, data = list(N = N, y = y, x = x),
  pars = c("b0", "b1", "phi"),
  iter = 500, chains = 4)

rstan::traceplot(m, inc_warmup = FALSE,nrow=3)
```
