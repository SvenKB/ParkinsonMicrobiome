...
fit3 <- brms::brm(richness~1+status+(1|study),
                 data=alpha_dat,
                 family = poisson(link = "log"),
                 iter=500)

fit4 <- brms::brm(richness~1+status+offset(log(N))+(1+status|study),
                 data=alpha_dat,
                 family = poisson(link = "log"),
                 iter=500)
...
study.draws <-  tidybayes::spread_draws(fit, study[study,], b_Intercept) %>% mutate(b_Intercept = r_Author + b_Intercept)
